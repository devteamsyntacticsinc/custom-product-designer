...
export async function GET(request: Request) {
  try {
    ...
    const [
      ...
      activityResult,
      topCustomersResult,
      ordersByProductTypeTimeSeriesResult,
      mostOrderedBrandResult,
      productTypesResult,
    ...
    const activity =
      activityResult.status === "fulfilled"
        ...
        : { activities: [], total: 0, totalPages: 0, currentPage: page };
    const topCustomers =
      topCustomersResult.status === "fulfilled" ? topCustomersResult.value : [];
    const ordersByProductTypeTimeSeries =
      ordersByProductTypeTimeSeriesResult.status === "fulfilled"
        ? ordersByProductTypeTimeSeriesResult.value
        : { data: [], types: [] };
